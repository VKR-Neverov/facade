...
func (ec *executionContext) _InvoiceMutations_createInvoice(ctx context.Context, field graphql.CollectedField, obj *model.InvoiceMutations) (ret graphql.Marshaler) {
	...
	resTmp, err := ec.ResolverMiddleware(ctx, func(rctx context.Context) (interface{}, error) {
		...
		if data, ok := tmp.(*model.CreateInvoicePayload); ok {
			return data, nil
		}
		return nil, fmt.Errorf(`unexpected type %T from directive, should be *github.com/fidesy-pay/facade/internal/pkg/model.CreateInvoicePayload`, tmp)
	})
	if err != nil {
		ec.Error(ctx, err)
		return graphql.Null
